...
document.addEventListener("DOMContentLoaded", () => {
   ...
   function dragOverPreviewImage() {
      ...
      tasksListElement.addEventListener(`dragover`, (evt) => {
         ...
         const isMoveable = activeElement !== currentElement &&
            currentElement.classList.contains(`preview__item`);
   ...
   document.addEventListener('click', (event) => {
      ...
      function clickContact() {
         ...
         if (target.tagName == 'INPUT' && target.closest('.contact__items')) {
            ...
            $(function () {
               $('.contact__telephone').mask('+7(000)000-00-00');
            });
         }
         ...
         if (target.closest('.contact__remove')) {
            ...
            if (contactItems.length != 1) {
               ...
               writeCookie('contact(new__resume)', JSON.stringify(contactArr), 30);
            }
   ...
   function loadContact() {
      if (readCookie('contact(new__resume)') !== undefined) {
         contactArr = JSON.parse(readCookie('contact(new__resume)'))
