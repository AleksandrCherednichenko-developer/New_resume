...
document.addEventListener("DOMContentLoaded", () => {
   ...
   if (readCookie('step-now(new__resume)') != undefined) {
      steps.forEach(step => {
         if (step.getAttribute('data-value') < readCookie('step-now(new__resume)')) {
      ...
      sections.forEach(section => {
         if (section.getAttribute('data-value') == readCookie('step-now(new__resume)')) {
            sections.forEach(section => {
